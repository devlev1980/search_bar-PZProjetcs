...
(async function build() {
  ...
  /**
   * Get the JS files from `./dist/searchbargoodWP/` directory and combine it into `bundle.js`
   * This will give support for angular v6 and all above versions.
   */

  try {
    let directoryPath = './dist/searchbargoodWP/';
    let files = await getFilesFromDirByType(directoryPath, 'js');
    files = files.map(file => directoryPath + file);
    await concat(files, './dist/searchbargoodWP/bundle.js');
  } catch (error) {
    console.log(error);
  }
})();
